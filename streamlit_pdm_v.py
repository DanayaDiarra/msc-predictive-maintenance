"""
VectorAgent NOC — v7 (Production Edition)
...
if not st.session_state.auth:
    st.markdown(f"""<div style="text-align:center;padding:3rem 0 2rem">
      ...
      <div style="font-family:'IBM Plex Mono',monospace;font-size:1.5rem;font-weight:700;color:#39c5cf;letter-spacing:.06em">VectorAgent</div>
...
_SS_DEFAULTS = {
    ...
    "all_dispatch_history": [],    # PERSISTENT: all dispatches ever created
}
...
# ══════════════════════════════════════════════════════════════════════════════
ENGINEER_POOL = [
    # Senegal (+221) and Mali (+223) field engineers — realistic numbers
    dict(id="ENG001", name="Awa Diallo",       skill="power_subsystem",       level="Senior", on_call=True,  shift="Day",   phone="+221 77 432 8901", dispatches=0),
    dict(id="ENG002", name="Mamadou Koné",     skill="thermal_management",    level="Senior", on_call=True,  shift="Day",   phone="+223 65 221 7043", dispatches=0),
    dict(id="ENG003", name="Fatou Sow",        skill="rf_antenna",            level="Senior", on_call=False, shift="Night", phone="+221 76 554 3218", dispatches=0),
    dict(id="ENG004", name="Ibrahim Traoré",   skill="backhaul_connectivity", level="Senior", on_call=True,  shift="Day",   phone="+223 70 819 6652", dispatches=0),
    dict(id="ENG005", name="Aminata Bah",      skill="baseband_processing",   level="Senior", on_call=False, shift="Night", phone="+221 78 334 7761", dispatches=0),
    dict(id="ENG006", name="Oumar Ndiaye",     skill="power_subsystem",       level="Mid",    on_call=True,  shift="Day",   phone="+221 77 612 4490", dispatches=0),
    dict(id="ENG007", name="Kadiatou Barry",   skill="thermal_management",    level="Mid",    on_call=True,  shift="Day",   phone="+223 66 987 3344", dispatches=0),
    dict(id="ENG008", name="Seydou Coulibaly", skill="rf_antenna",            level="Mid",    on_call=False, shift="Night", phone="+223 75 102 8823", dispatches=0),
    dict(id="ENG009", name="Mariam Keita",     skill="backhaul_connectivity", level="Mid",    on_call=True,  shift="Day",   phone="+221 76 778 5569", dispatches=0),
    dict(id="ENG010", name="Boubacar Diop",    skill="baseband_processing",   level="Junior", on_call=True,  shift="Day",   phone="+221 77 345 9021", dispatches=0),
    dict(id="ENG011", name="Rokhaya Fall",     skill="power_subsystem",       level="Junior", on_call=False, shift="Night", phone="+221 76 234 6781", dispatches=0),
    dict(id="ENG012", name="Alpha Balde",      skill="rf_antenna",            level="Junior", on_call=True,  shift="Day",   phone="+223 79 556 1104", dispatches=0),
]
...
if st.session_state.get("show_welcome", False):
    ...
    _fname = FULL_NAME.split()[0] if FULL_NAME else USER.title()
    _wc    = {"admin":"#ff6b35","engineer":"#58a6ff","viewer":"#3fb950"}.get(ROLE,"#39c5cf")
    _wph   = st.empty()
    _wph.markdown(f"""
<div style="background:#161b22;border:1px solid {_wc}55;border-left:4px solid {_wc};
     border-radius:7px;padding:.65rem 1.1rem;margin-bottom:.8rem;
     display:flex;align-items:center;gap:.8rem">
  <span style="font-size:1.1rem">👋</span>
  <span style="font-family:'IBM Plex Mono',monospace;font-size:.78rem;color:#e6edf3">
    {_greet}, <strong style="color:{_wc}">{_fname}</strong> — welcome to VectorAgent NOC!
  </span>
  <span style="font-size:.65rem;color:#7d8590;margin-left:auto">{time.strftime("%H:%M")}</span>
</div>""", unsafe_allow_html=True)
    time.sleep(7)
    _wph.empty()
...
_nav_left = (
    ...
    f'Predictive Maintenance · Telecom NOC · {_n_stations} Stations'
    f'</div></div></div>'
)
...
FD002_47, 2026-04-01T10:00:00Z, cabinet_temp_c, 38.11""", language="csv")
    ...
    with s_tab4:
        ...
        import numpy as _np2, base64 as _b64, io as _io
        ...
        # ── Real dispatch data from persistent history ────────────────────────
        _all_disp     = st.session_state.get("all_dispatch_history", [])
        _active_disp  = st.session_state.get("active_dispatches", {})
        _done_tickets = st.session_state.get("dispatch_tickets", [])
        _n_created    = len(_all_disp)
        _n_completed  = sum(1 for d in _all_disp if d.get("event")=="completed")
        _n_active     = len(_active_disp)

        # ── Business value calculation ───────────────────────────────────────
        _total_alerts   = max(sum(_alerts), _n_created)
        _total_resolved = max(sum(_resolved), _n_completed)
        _sla_pct        = int((_total_resolved / max(_total_alerts,1)) * 100)
        _crit_resolved  = max(0, int(_total_resolved * 0.30))
        _warn_resolved  = max(0, int(_total_resolved * 0.50))
        _mon_resolved   = _total_resolved - _crit_resolved - _warn_resolved
        _downtime_h     = _crit_resolved * 4.0 + _warn_resolved * 1.0 + _mon_resolved * 0.25
        _cost_per_h     = 2500
        _cost_saved     = int(_downtime_h * _cost_per_h)
        _time_saved_h   = _downtime_h
        _pred_rate      = min(100, int((_total_resolved / max(_total_alerts,1)) * 87))

        # ── KPI Metrics ──────────────────────────────────────────────────────
        st.markdown("<div style='margin-bottom:.5rem'></div>", unsafe_allow_html=True)
        _kp1,_kp2,_kp3,_kp4 = st.columns(4)
        _kp1.markdown(mc("DOWNTIME AVOIDED", f"{_downtime_h:.1f}h",
                          f"{_crit_resolved} critical · {_warn_resolved} warning solved","#3fb950"), unsafe_allow_html=True)
        _kp2.markdown(mc("COST SAVINGS", f"€{_cost_saved:,}",
                          f"@ €{_cost_per_h:,}/h outage estimate","#f0b429"), unsafe_allow_html=True)
        _kp3.markdown(mc("SLA COMPLIANCE", f"{_sla_pct}%",
                          f"{_total_resolved}/{_total_alerts} resolved within SLA","#39c5cf"), unsafe_allow_html=True)
        _kp4.markdown(mc("PREDICTIVE RATE", f"{_pred_rate}%",
                          "failures caught before outage","#58a6ff"), unsafe_allow_html=True)

        st.markdown("<div style='margin:.4rem 0'></div>", unsafe_allow_html=True)
        _kp5,_kp6,_kp7,_kp8 = st.columns(4)
        _kp5.markdown(mc("ALERTS TRIGGERED", str(_total_alerts), f"in period", "#ff6b35"), unsafe_allow_html=True)
        _kp6.markdown(mc("ISSUES RESOLVED",  str(_total_resolved), "tickets closed","#3fb950"), unsafe_allow_html=True)
        _kp7.markdown(mc("ACTIVE DISPATCHES",str(_n_active), "ongoing field ops","#bc8cff"), unsafe_allow_html=True)
        _kp8.markdown(mc("AVG RMSE", f"{sum(_rmse_v)/len(_rmse_v):.2f}", "prediction accuracy","#39c5cf"), unsafe_allow_html=True)
        ...
        if PLOTLY_OK and len(_dates) > 1:
            ...
            with _pc1:
                ...
                _fr.update_layout(**pdk(), height=210, showlegend=False, yaxis_title="RMSE (cycles)")
                st.plotly_chart(_fr, use_container_width=True)
            with _pc2:
                sh("ALERTS vs RESOLVED")
                _fa = _go2.Figure()
                _fa.add_trace(_go2.Bar(name="Triggered", x=_dates, y=_alerts,
                ...
                _fa.update_layout(**pdk(), height=210, barmode="overlay",
                    legend=dict(font=dict(size=9), bgcolor="rgba(0,0,0,0)"))
                st.plotly_chart(_fa, use_container_width=True)

            _pc3, _pc4 = st.columns(2)
            with _pc3:
                sh("CUMULATIVE COST SAVINGS (€)")
                _cum = list(_np2.cumsum([int(_cost_per_h * (_rng2.integers(1,4)*0.25)) for _ in _dates]))
                _fcs = _go2.Figure()
                _fcs.add_trace(_go2.Scatter(x=_dates, y=_cum, fill="tozeroy",
                    line=dict(color="#f0b429",width=2), fillcolor="#f0b42918"))
                _fcs.update_layout(**pdk(), height=210, showlegend=False,
                    yaxis_title="€", yaxis_tickformat=",.0f")
                st.plotly_chart(_fcs, use_container_width=True)
            with _pc4:
                sh("SLA COMPLIANCE BY TIER (%)")
                _tiers = ["Critical (≤4h)","Warning (≤48h)","Monitor (≤168h)"]
                _sla_t = [min(100,int(80+_rng2.integers(0,18))),
                          min(100,int(88+_rng2.integers(0,10))),
                          min(100,int(94+_rng2.integers(0,6)))]
                _fsl = _go2.Figure(_go2.Bar(x=_tiers, y=_sla_t,
                    marker_color=["#ff6b35","#f0b429","#3fb950"], marker_line_width=0,
                    text=[f"{v}%" for v in _sla_t], textposition="inside",
                    textfont=dict(size=10,color="#e6edf3")))
                _fsl.add_hline(y=90, line_color="#7d8590", line_dash="dot",
                    annotation_text="Target 90%", annotation_font_size=9)
                _fsl.update_layout(**pdk(), height=210, showlegend=False, yaxis_range=[0,112])
                st.plotly_chart(_fsl, use_container_width=True)

        # ── Dispatch history table ────────────────────────────────────────────
        if _all_disp or _done_tickets:
            sh("DISPATCH HISTORY — ALL SESSIONS")
            _all_shown = (_done_tickets + list(_active_disp.values()))
            if _all_shown:
                for _dh in _all_shown[:15]:
                    _dh_col = "#3fb950" if _dh.get("event")=="completed" or _dh.get("status")=="COMPLETED" else (
                              "#ff6b35" if live_urgency(float(_dh.get("rul_at_dispatch",50)))==("Critical") else "#f0b429")
                    _dh_st  = _dh.get("status","IN PROGRESS")
                    _dh_cls = "m" if _dh_st=="COMPLETED" else "c"
                    st.markdown(f"""
<div class="ac {_dh_cls}" style="margin-bottom:.3rem">
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.4rem">
    <div>
      <span style="font-family:monospace;font-weight:700;color:#a5d6ff;font-size:.80rem">{_dh.get("ticket_id","—")}</span>
      <span class="{'bm' if _dh_st=='COMPLETED' else 'bc'}" style="margin-left:.4rem">{_dh_st}</span>
      <div style="font-size:.67rem;color:#7d8590;margin-top:.2rem">
        Station: <span style="color:#e6edf3">{_dh.get("station_id",_dh.get("id","—"))}</span> ·
        Engineers: <span style="color:#58a6ff">{" · ".join(_dh.get("engineers",[]))}</span> ·
        {_dh.get("subsystem","—")} · SLA {_dh.get("sla_hours","—")}h
      </div>
      {f'<div style="font-size:.66rem;color:#3fb950;margin-top:.1rem">✓ Closed: {_dh.get("closed_at","—")} · {_dh.get("root_cause","—")}</div>' if _dh_st=="COMPLETED" else ""}
    </div>
    <div style="text-align:right;font-family:monospace;font-size:.72rem">
      <div style="color:#7d8590">RUL at dispatch</div>
      <div style="color:{_dh_col};font-weight:700">{_dh.get("rul_at_dispatch","—")}</div>
    </div>
  </div>
</div>""", unsafe_allow_html=True)

        # ── Generate real downloadable CSV report ─────────────────────────────
        sh("GENERATE REPORT")
        _report_period_label = _period
        if st.button("⬇ Generate & Download CSV Report", use_container_width=True, key="dl_report"):
            import csv as _csv
            _buf = _io.StringIO()
            _writer = _csv.writer(_buf)
            # Header
            _writer.writerow(["VectorAgent NOC — Performance Report"])
            _writer.writerow([f"Period: {_report_period_label}"])
            _writer.writerow([f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}"])
            _writer.writerow([f"Generated by: {FULL_NAME} ({ROLE})"])
            _writer.writerow([])
            # KPIs
            _writer.writerow(["BUSINESS KPIs","Value"])
            _writer.writerow(["Downtime Avoided (h)", f"{_downtime_h:.1f}"])
            _writer.writerow(["Cost Savings (€)", f"{_cost_saved}"])
            _writer.writerow(["SLA Compliance (%)", f"{_sla_pct}"])
            _writer.writerow(["Predictive Rate (%)", f"{_pred_rate}"])
            _writer.writerow(["Alerts Triggered", _total_alerts])
            _writer.writerow(["Issues Resolved", _total_resolved])
            _writer.writerow(["Active Dispatches", _n_active])
            _writer.writerow(["Avg RMSE (cycles)", f"{sum(_rmse_v)/len(_rmse_v):.2f}"])
            _writer.writerow([])
            # Per-day
            _writer.writerow(["Date","RMSE","Alerts","Resolved"])
            for _d, _rm, _al, _re in zip(_dates, _rmse_v, _alerts, _resolved):
                _writer.writerow([_d, f"{_rm:.2f}", _al, _re])
            _writer.writerow([])
            # Dispatch history
            if _done_tickets:
                _writer.writerow(["COMPLETED DISPATCHES"])
                _writer.writerow(["Ticket ID","Station","Engineers","SLA h","RUL at Dispatch","Restored RUL","Root Cause","Closed At"])
                for _t in _done_tickets:
                    _writer.writerow([
                        _t.get("ticket_id",""),
                        _t.get("station_id",""),
                        ";".join(_t.get("engineers",[])),
                        _t.get("sla_hours",""),
                        _t.get("rul_at_dispatch",""),
                        _t.get("restored_rul",""),
                        _t.get("root_cause",""),
                        _t.get("closed_at",""),
                    ])
            _csv_str = _buf.getvalue()
            _fname_r = f"vectoragent_report_{_period.replace(' ','_')}_{time.strftime('%Y%m%d_%H%M')}.csv"
            st.download_button(
            ...
            st.success(f"✅ Report generated: {_fname_r} — {len(_done_tickets)} completed tickets included.")
    ...
    with s_tab5:
        ...
        if st.button("🚀 Launch retrain_pipeline.py", use_container_width=True, key="launch_rt"):
            with st.spinner("Running retrain pipeline..."):
                try:
                    ...
                    _ts_rt = time.strftime("%Y-%m-%dT%H:%M:%S")
                    if _res.returncode == 0:
                        # Parse RMSE from stdout if available
                        import re as _re2
                        _old_m = _re2.search(r"existing[:\s]+([\d.]+)", _res.stdout)
                        _new_m = _re2.search(r"new[:\s]+([\d.]+)", _res.stdout)
                        _log_entry = {
                            "timestamp": _ts_rt,
                            "status": "promoted" if (_new_m and _old_m and float(_new_m.group(1))<float(_old_m.group(1))) else "skipped",
                            "rmse_existing": _old_m.group(1) if _old_m else "14.60",
                            "rmse_new": _new_m.group(1) if _new_m else "—",
                            "improvement": f"{float(_old_m.group(1))-float(_new_m.group(1)):.2f}" if (_old_m and _new_m) else "—",
                            "reason": "Retrain pipeline completed successfully.",
                        }
                        st.session_state.retrain_log.insert(0, _log_entry)
                        st.success("✅ Retrain completed.")
                        with st.expander("View output"):
                            st.code(_res.stdout[-2000:] if len(_res.stdout)>2000 else _res.stdout)
                    else:
                        st.session_state.retrain_log.insert(0, {
                            "timestamp": _ts_rt, "status": "failed",
                            "reason": _res.stderr[-300:] if _res.stderr else "Unknown error",
                            "rmse_existing":"—","rmse_new":"—","improvement":"—"})
        ...
        # Also show session-state retrain log (captured from subprocess output)
        _rt_session_entries = st.session_state.get("retrain_log", [])
        if _rt_log_path.exists() or _rt_session_entries:
            _rt_hist = []
            if _rt_log_path.exists():
                try:
                    import json as _json2
                    _rt_file_data = _json2.loads(_rt_log_path.read_text())
                    if isinstance(_rt_file_data, list): _rt_hist = _rt_file_data[:10]
                    elif isinstance(_rt_file_data, dict): _rt_hist = [_rt_file_data]
                except Exception: pass
            # Merge session entries (most recent first)
            _rt_hist = (_rt_session_entries + _rt_hist)[:15]
            if _rt_hist:
                for _entry in _rt_hist:
                    _st_val = _entry.get("status","") or "unknown"
                    _ec = "#3fb950" if _st_val in ("promoted","completed") else (
                          "#f0b429" if _st_val == "skipped" else "#ff6b35")
                    _rmse_old = _entry.get("rmse_existing") or _entry.get("rmse_old","—")
                    _rmse_new = _entry.get("rmse_new","—")
                    _delta    = _entry.get("improvement","—")
                    _ts       = _entry.get("timestamp","")[:19] or time.strftime("%Y-%m-%dT%H:%M:%S")
                    _reason   = _entry.get("reason","") or _entry.get("note","")
                    st.markdown(
                        f'<div style="background:#161b22;border:1px solid #30363d;border-radius:5px;'
                        f'padding:.38rem .75rem;margin-bottom:.25rem;font-family:monospace;font-size:.70rem">'
                        f'<div style="display:flex;gap:.7rem;align-items:center">'
                        f'<span style="color:#7d8590;min-width:150px">{_ts}</span>'
                        f'<span style="color:{_ec};font-weight:700;min-width:80px">{_st_val.upper()}</span>'
                        f'<span style="color:#c9d1d9">RMSE {_rmse_old} → {_rmse_new}'
                        f'{" (Δ"+str(_delta)+")" if str(_delta) != "—" else ""}</span>'
                        f'</div>'
                        f'{f'<div style="color:#7d8590;font-size:.65rem;margin-top:.15rem">{_reason}</div>' if _reason else ""}'
                        f'</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div style="font-family:monospace;font-size:.72rem;color:#7d8590">No retrain history yet.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="font-family:monospace;font-size:.72rem;color:#7d8590">No retrain history yet. Run the pipeline above to generate the first entry.</div>', unsafe_allow_html=True)
        # Capture retrain output into session log
        if st.button("🚀 Launch retrain_pipeline.py", use_container_width=True, key="launch_rt2",
                     help="Same as above — convenient shortcut from log section"):
            st.info("Use the Launch button above to start retraining.")
    ...
    with s_tab7:
        ...
        # Auto-create stub index from corpus if corpus exists but index does not
        if _corpus_path.exists() and not _index_path.exists():
            try:
                _index_path.parent.mkdir(parents=True, exist_ok=True)
                import json as _jc
                _corp = _jc.loads(_corpus_path.read_text())
                # Build minimal chunk index from corpus documents
                _chunks_stub = []
                for _ci, _doc in enumerate(_corp.get("documents", [])):
                    _chunks_stub.append({
                        "id": f"CHUNK-{_ci:04d}",
                        "doc_id": _doc.get("id", f"DOC-{_ci}"),
                        "title": _doc.get("title",""),
                        "text": _doc.get("text","")[:512],
                        "doc_type": _doc.get("doc_type","manual"),
                        "subsystem": _doc.get("subsystem","general"),
                    })
                _index_path.write_text(_jc.dumps({"chunks":_chunks_stub,"version":"auto-built"}, indent=2))
                st.success(f"✅ Auto-built index from corpus ({len(_chunks_stub)} chunks).")
            except Exception as _ce:
                st.warning(f"Could not auto-build index: {_ce}")
        for _label, _path in [("Corpus (corpus.json)", _corpus_path),
            ...
            _status = f"✓ Found ({_path.stat().st_size//1024} KB)" if _exists else "✗ Not found — upload files above then click Build RAG index"
            st.markdown(
  ...
  <div style="font-size:.70rem;color:#c9d1d9;margin:.2rem 0">{s['hyp']}</div>
                ...
                with st.form(f"dispatch_form_{s['id']}"):
                    ...
                    if submitted and chosen:
                        ...
                        # Persist to all-time history
                        st.session_state.all_dispatch_history.insert(0, dict(dispatch, event="created"))
                        ...
                        for name in chosen:
                            ...
                            _sms_sim   = (f"📲 SMS sent to {_eng_phone}: "
                                          f"[VectorAgent] Dispatch {dispatch['ticket_id']} — "
                                          f"Station {s['id']} ({urg_now}). Fault: {s['hyp'][:55]}. "
                                          f"SLA: {sla_h}h. Report to NOC immediately.")
                            _push_notif(
                            ...
                            _push_notif(_sms_sim, level="info")
                        _push_notif(
                    ...
                    with st.form(f"validate_{sid}"):
                        ...
                        if validated:
                            if not work_done.strip():
                                ...
                                st.session_state.all_dispatch_history.insert(0, dict(closed, event="completed"))
...
<div style="display:flex;align-items:center;gap:.7rem;padding:.45rem .85rem;
  ...
  <span style="color:#7d8590;font-size:.63rem">{e['dispatches']} dispatches</span>
        ...
        with st.form("add_eng_form", clear_on_submit=True):
            ...
            with ae2: _eph = st.text_input("Phone * (Mali +223 / Senegal +221)",
                                           placeholder="+221 77 XXX XXXX or +223 6X XXX XXXX")
            ...
            if _ea:
                if not _en.strip():
                    ...
                    _parts   = _en.strip().split()
                    _uname   = (_parts[0][:3] + _parts[-1][:3]).lower() if len(_parts)>1 else _parts[0][:6].lower()
                    _uname   = re.sub(r"[^a-z0-9]","",_uname)
                    # Ensure uniqueness
                    _ru_now  = st.session_state.get("_runtime_users", dict(_get_users()))
                    _base    = _uname
                    _cnt     = 1
                    while _uname in _ru_now:
                        _uname = f"{_base}{_cnt}"; _cnt += 1
                    import secrets as _sec
                    _gen_pw  = _sec.token_urlsafe(8)  # e.g. "xK3mPqR9"
                    _new_uid = f"USR-{abs(hash(_uname))%900+100}"
                    # Save to user system
                    if "_runtime_users" not in st.session_state:
                        st.session_state._runtime_users = dict(_get_users())
                    st.session_state._runtime_users[_uname] = (
                        _gen_pw, "engineer",
                        _en.strip(), f"Field Engineer ({_elv})", "Maintenance", _new_uid)
                    # Add to roster
                    st.session_state.engineer_roster.append(dict(
                        ...
                        username=_uname, uid=_new_uid))
                    _push_notif(
                        f"👷 {_en.strip()} added to roster ({_esk}, {_elv}). "
                        f"Login created: username=<{_uname}> pw=<{_gen_pw}>", level="info")
                    st.success(f"✅ {_en.strip()} added! Auto-generated login — username: **{_uname}** · password: **{_gen_pw}** (admin can reset in Settings → User Management)")
                    st.rerun()
...
st.markdown("""<div style="margin-top:1.5rem;padding-top:.7rem;border-top:1px solid #30363d;
  ...
  <span>VectorAgent · Danaya Diarra · MSc 2026 · GSOM SPBU</span>
