...
with st.sidebar:
    ...
    if IS_ENG:
        ...
        st.markdown("---")
        st.markdown("### 🔑 AI Chatbot Keys")
        st.caption("All free options — first working key is used in priority order.")
        with st.expander("🔵 Anthropic Claude", expanded=False):
            st.caption("console.anthropic.com — sk-ant-...")
            _a = st.text_input("Key", type="password", placeholder="sk-ant-...",
                                label_visibility="collapsed",
                                value=st.session_state.get("_rt_ant_key",""),
                                key="sb_ant")
            if st.button("💾 Save Anthropic", use_container_width=True, key="sv_ant"):
                st.session_state._rt_ant_key = _a.strip(); st.success("Saved")
        with st.expander("🟢 Groq · LLaMA 3.3 70B (fastest free)", expanded=False):
            st.caption("console.groq.com — gsk_...")
            _g = st.text_input("Key", type="password", placeholder="gsk_...",
                                label_visibility="collapsed",
                                value=st.session_state.get("_rt_GROQ_API_KEY",""),
                                key="sb_groq")
            if st.button("💾 Save Groq", use_container_width=True, key="sv_groq"):
                st.session_state._rt_GROQ_API_KEY = _g.strip(); st.success("Saved")
        with st.expander("🟢 OpenRouter · DeepSeek / Llama (free tier)", expanded=False):
            st.caption("openrouter.ai — sk-or-...")
            _o = st.text_input("Key", type="password", placeholder="sk-or-...",
                                label_visibility="collapsed",
                                value=st.session_state.get("_rt_OPENROUTER_API_KEY",""),
                                key="sb_or")
            _om = st.selectbox("Model", [
                "deepseek/deepseek-chat-v3-0324:free",
                "meta-llama/llama-3.3-70b-instruct:free",
                "mistralai/mistral-7b-instruct:free",
                "google/gemma-3-27b-it:free",
            ], key="sb_or_model")
            if st.button("💾 Save OpenRouter", use_container_width=True, key="sv_or"):
                st.session_state._rt_OPENROUTER_API_KEY = _o.strip()
                st.session_state._rt_OR_MODEL = _om; st.success("Saved")
        with st.expander("🟡 HuggingFace Inference · Zephyr-7B", expanded=False):
            st.caption("huggingface.co/settings/tokens — hf_...")
            _h = st.text_input("Token", type="password", placeholder="hf_...",
                                label_visibility="collapsed",
                                value=st.session_state.get("_rt_HF_TOKEN",""),
                                key="sb_hf")
            if st.button("💾 Save HF Token", use_container_width=True, key="sv_hf"):
                st.session_state._rt_HF_TOKEN = _h.strip(); st.success("Saved")
        _eng_active = []
        if _get_ant_key():              _eng_active.append("🔵 Claude")
        if _get_free_key("GROQ_API_KEY"):      _eng_active.append("🟢 Groq")
        if _get_free_key("OPENROUTER_API_KEY"): _eng_active.append("🟢 OR")
        if _get_free_key("HF_TOKEN"):           _eng_active.append("🟡 HF")
        _ea_str = "  ·  ".join(_eng_active) if _eng_active else "📚 Rule-based only"
        _ea_col = "#3fb950" if _eng_active else "#f0b429"
        st.markdown(f'<div style="font-family:monospace;font-size:.62rem;color:{_ea_col};margin-top:.3rem">Active: {_ea_str}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.chat_thinking and st.session_state.chat_history:
        ...
        with st.spinner("Thinking…"):
            ...
            answer = None; engine_used = "Rule-based"; _errs = []
            # 1. Anthropic Claude
            if not answer and ant_key:
                answer, _e = call_claude(ant_key, prev, sys_p)
                if answer: engine_used = "🔵 Claude Haiku (Anthropic)"
                elif _e: _errs.append(_e)
            # 2. Groq — free, LLaMA 3.3 70B
            _gk = _get_free_key("GROQ_API_KEY")
            if not answer and _gk:
                answer, _e = _call_groq(_gk, prev, sys_p)
                if answer: engine_used = "🟢 Groq · LLaMA 3.3 70B (free)"
                elif _e: _errs.append(_e)
            # 3. OpenRouter — DeepSeek v3 free
            _ok = _get_free_key("OPENROUTER_API_KEY")
            _om = st.session_state.get("_rt_OR_MODEL","deepseek/deepseek-chat-v3-0324:free")
            if not answer and _ok:
                answer, _e = _call_openrouter(_ok, prev, sys_p, model=_om)
                if answer: engine_used = f"🟢 OpenRouter · {_om.split('/')[1][:18]} (free)"
                elif _e: _errs.append(_e)
            # 4. HuggingFace Inference
            _hk = _get_free_key("HF_TOKEN")
            if not answer and _hk:
                answer, _e = _call_hf(_hk, prev, sys_p)
                if answer: engine_used = "🟡 HF Inference · Zephyr-7B (free)"
                elif _e: _errs.append(_e)
            ...
            if not answer:
                ...
                if rb:
                    answer = rb
                    engine_used = f"📚 Rule-based KB{'  |  '+_err[:80] if _err else ''}"
                else:
                    err_html = ""
                    if _errs:
                        err_html = ("<details><summary style='cursor:pointer;font-size:.68rem;color:#7d8590'>"
                                    f"▸ Debug ({len(_errs)} API errors)</summary>"
                                    "<div style='font-size:.65rem;color:#f0b429;line-height:1.6'>"
                                    + "<br>".join(_errs) + "</div></details>")
                    answer = ("No specific rule matched that query.<br><br>"
                              "<strong style='color:#39c5cf'>Try asking about:</strong><br>"
                              "• Alarm codes: PWR-001/004, COOL-001/003, RF-001/005, BKH-001, BBU-003<br>"
                              "• Procedures: fan replacement, connector inspection, BBU upgrade, OTDR<br>"
                              "• RUL urgency thresholds · G.826 ESR/SESR thresholds · VSWR/PIM testing<br><br>"
                              "<em style='color:#7d8590'>Add any free API key in sidebar for AI answers.</em>"
                              + err_html)
...
<div style="background:linear-gradient(135deg,#1c2333,#161b22);border:1px solid #39c5cf44;
     border-radius:12px;padding:1.4rem 1.8rem;margin-bottom:1.2rem">
  <div style="font-family:'IBM Plex Mono',monospace;font-size:1.2rem;font-weight:700;color:#e6edf3;margin-bottom:.3rem">
    📖 TeleMaint AI — <span style="color:#39c5cf">System User Guide</span>
  </div>
  <div style="font-size:.80rem;color:#c9d1d9;line-height:1.7">
    Complete reference for operators, field engineers, and thesis evaluators.
    Covers system architecture, all metrics, navigation pages, chatbot setup, governance model, and deployment.
  </div>
  <div style="font-size:.68rem;color:#7d8590;font-family:'IBM Plex Mono',monospace;margin-top:.35rem">
    Danaya Diarra · MSc Thesis 2026 · GSOM SPBU · Agentic AI for Predictive Maintenance
  </div>
</div>""", unsafe_allow_html=True)

    _ug_main = st.tabs(["🏗 System & Navigation", "🤖 AI Chatbot Setup", "⚡ Live Mode", "📊 Metrics Reference", "🔧 Deployment"])

    # ──────────────────────────────────────────────────────────────────────────
    with _ug_main[0]:
        sh("THREE-LAYER AGENTIC ARCHITECTURE")
        _layers = [
            ("Layer 1 — Perception","#58a6ff","XGBoost v2 Final",
             "Ingests 21 multivariate sensor channels mapped to telecom KPIs. Outputs RUL prediction ± CI, "
             "urgency tier, and top contributing features per station. "
             "15,000 gradient-boosted trees trained on all 4 NASA C-MAPSS sub-datasets with "
             "exp(α=3) near-failure sample weighting (RUL≤30 samples weighted ~4×). "
             "RMSE=14.60 all-4 · RMSE=12.77 FD001+FD003 · R²=0.874.",
             ["21 sensor channels → telecom KPI mapping (voltage, temp, fan speed, VSWR, latency…)",
              "exp(α=3) weighting: critical zone (RUL≤30) accuracy prioritised",
              "Per-subset: FD001=12.31 · FD002=15.87 · FD003=13.23 · FD004=16.99",
              "Confidence interval: ±3.13 cycles (1σ bootstrap)"]),
            ("Layer 2 — Knowledge Grounding","#39c5cf","Hybrid RAG Pipeline",
             "Retrieves ranked evidence from 33-chunk telecom corpus (vendor manuals, SOPs, alarm dicts, "
             "tickets, 3GPP/ITU specs, FMEA tables, decision trees). "
             "Hybrid sparse (TF-IDF) + dense (TruncatedSVD 64-dim LSA) retrieval fused via "
             "Reciprocal Rank Fusion k=60. Every LLM claim cites a [DOC-ID]. "
             "Grounding=1.00 · Hallucination=0.00 · Latency=9ms.",
             ["BM25-proxy + 64-dim LSA → RRF k=60 fusion",
              "Metadata boost: subsystem, doc_type, urgency alignment",
              "Top-5 evidence chunks with full [DOC-ID] provenance",
              "Coverage score = fraction of top-K chunks matching alerted subsystem"]),
            ("Layer 3 — Reasoning-to-Action","#bc8cff","ReAct + Pre-Planning",
             "Three specialised agents: ① Diagnostic Agent generates root-cause hypotheses with evidence citations; "
             "② Planning Agent evaluates actions against SLA/risk/cost constraints; "
             "③ Execution Agent invokes governance-gated tools. "
             "Pattern: Observe → Reason → Act → Learn. E2E latency: 33ms.",
             ["Diagnostic Agent: hypothesis + confidence from RAG evidence",
              "Planning Agent: action ranking against SLA, risk tier, cost budget",
              "Execution Agent: query_cmdb · open_ticket · schedule_dispatch · remote_command",
              "Tiered governance: AUTO (Tier 1) · TIMEOUT (Tier 2) · HUMAN (Tier 3)"]),
        ]
        for lbl, col, model, desc, bullets in _layers:
            st.markdown(f"""
<div style="background:#161b22;border:1px solid {col}44;border-radius:8px;padding:.95rem 1.15rem;margin-bottom:.65rem">
  <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.4rem">
    <span style="font-size:.85rem;font-weight:700;color:{col};font-family:'IBM Plex Mono',monospace">{lbl}</span>
    <span style="font-size:.67rem;color:#7d8590;background:{col}22;padding:1px 7px;border-radius:3px;font-family:'IBM Plex Mono',monospace">{model}</span>
  </div>
  <div style="font-size:.78rem;color:#c9d1d9;line-height:1.72;margin-bottom:.45rem">{desc}</div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:.2rem">
    {"".join(f'<div style="font-size:.71rem;color:#7d8590;font-family:monospace;padding:.15rem 0">▶ {b}</div>' for b in bullets)}
  </div>
</div>""", unsafe_allow_html=True)

        sh("NAVIGATION PAGES QUICK REFERENCE")
        _pg_guide = [
            ("🔴 Live Fleet Monitor","#ff6b35","Primary NOC operational screen",
             "Live RUL countdown for all 15 stations, per-station sensor sparklines, auto-refresh (5–60s), alert log for urgency escalations. Use during active NOC shifts.",
             "Enable Live Mode toggle in sidebar · Reset session clock resets all RUL counters"),
            ("🏠 Fleet Overview","#58a6ff","Static snapshot + Plotly analytics",
             "XGBoost v2 base predictions with CI error bars, diagnostic quality radar chart, pipeline latency breakdown. Best for reports and presentations.",
             "RUL error bars show confidence intervals · Radar chart compares all stations"),
            ("🔍 Station Detail","#39c5cf","Per-station deep-dive",
             "Live RUL gauge with animated needle, live trajectory chart with NOW marker, feature importance bars, root-cause hypothesis, fault component, alarm code, governance-gated actions.",
             "Gauge needle updates in Live Mode · Feature bars are gain-based from fitted model"),
            ("🚚 Dispatch & Roster","#f0b429","Field engineer management",
             "Live dispatch queue sorted by urgency. Add/remove engineers from roster, set skill specialisations, approve TIMEOUT-tier dispatches, mark jobs complete.",
             "Approve button available for TIMEOUT tier actions · Admin/Engineer only"),
            ("🤖 Engineer Chatbot","#bc8cff","AI maintenance assistant",
             "Multi-engine chatbot grounded in telecom knowledge base. Priority: Anthropic → Groq → OpenRouter → HF → Rule-based. Requires Engineer or Admin role.",
             "All engines are free options · Rule-based works without any API key"),
            ("🧠 Pipeline Intelligence","#3fb950","RAG evidence + reasoning trace",
             "Two sub-tabs: RAG Evidence (chunks, RRF scores, coverage) and Agent Reasoning (7-step trace, governance tier, memory store JSON). Key for thesis defence.",
             "Reasoning trace is the full Observe→Reason→Act→Learn chain"),
            ("📊 Results & Ablation","#7d8590","Model benchmarks + ablation",
             "Full per-subset RMSE table (all 4 sub-datasets × all models) and 5-configuration ablation study (A→E) with grounding/hallucination charts.",
             "Ablation shows incremental value of each pipeline layer · B vs A = −8.2% RMSE"),
            ("👤 User Management","#ff6b35","Admin only — CRUD users",
             "Add, remove, change roles in-session. Seeded from .streamlit/secrets.toml [users]. Changes persist for browser session. Cannot remove own account.",
             "Prefix in username determines role: admin_ / eng_ / viewer_"),
        ]
        for pname, pcol, ptype, pdesc, ptip in _pg_guide:
            st.markdown(f"""
<div style="display:grid;grid-template-columns:200px 1fr;gap:.7rem;padding:.5rem .75rem;
     background:#161b22;border:1px solid #30363d;border-left:3px solid {pcol};
     border-radius:6px;margin-bottom:.3rem;align-items:start">
  <div>
    <div style="font-size:.78rem;font-weight:700;color:{pcol};font-family:'IBM Plex Mono',monospace">{pname}</div>
    <div style="font-size:.64rem;color:#7d8590;margin-top:.1rem">{ptype}</div>
  </div>
  <div>
    <div style="font-size:.75rem;color:#c9d1d9;margin-bottom:.2rem">{pdesc}</div>
    <div style="font-size:.67rem;color:#7d8590;font-family:'IBM Plex Mono',monospace">✓ {ptip}</div>
  </div>
</div>""", unsafe_allow_html=True)

    # ──────────────────────────────────────────────────────────────────────────
    with _ug_main[1]:
        sh("FREE AI CHATBOT OPTIONS — PRIORITY ORDER")
        _apis = [
            ("1","🟢","Groq","Fastest, completely free","console.groq.com",
             "gsk_...","GROQ_API_KEY","LLaMA 3.3 70B","~400ms","Unlimited free tier","#3fb950"),
            ("2","🔵","Anthropic Claude","Highest quality","console.anthropic.com",
             "sk-ant-...","ANTHROPIC_API_KEY","claude-haiku-4-5-20251001","~1.2s","Free credits on signup","#58a6ff"),
            ("3","🟢","OpenRouter","Multiple free models","openrouter.ai",
             "sk-or-...","OPENROUTER_API_KEY","DeepSeek v3 / Llama 3.3 / Gemma 3 / Mistral 7B","~700ms","Free tier available","#39c5cf"),
            ("4","🟡","HuggingFace","Free with account","huggingface.co/settings/tokens",
             "hf_...","HF_TOKEN","Zephyr-7B-beta","~3s","Free, cold starts","#f0b429"),
            ("5","📚","Rule-based KB","Always works, no key","Built-in",
             "none","—","Telecom maintenance knowledge base","<1ms","Zero cost, always available","#7d8590"),
        ]
        for rank, em, name, tagline, url, prefix, env_var, model, latency, cost, col in _apis:
            st.markdown(f"""
<div style="background:#161b22;border:1px solid {col}44;border-radius:7px;padding:.85rem 1.05rem;margin-bottom:.5rem">
  <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.4rem;flex-wrap:wrap">
    <span style="font-size:.80rem;font-weight:700;color:{col};font-family:'IBM Plex Mono',monospace">#{rank} {em} {name}</span>
    <span style="font-size:.72rem;color:#c9d1d9">{tagline}</span>
    <a href="https://{url}" style="font-size:.68rem;color:#39c5cf;margin-left:auto">{url}</a>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:.5rem;font-size:.71rem;font-family:'IBM Plex Mono',monospace">
    <div><span style="color:#7d8590">Model: </span><span style="color:#c9d1d9">{model}</span></div>
    <div><span style="color:#7d8590">Latency: </span><span style="color:#c9d1d9">{latency}</span></div>
    <div><span style="color:#7d8590">Cost: </span><span style="color:{col}">{cost}</span></div>
    <div><span style="color:#7d8590">Key prefix: </span><span style="color:#f0b429">{prefix}</span>
         <span style="color:#7d8590"> · Secret: </span><span style="color:#39c5cf">{env_var}</span></div>
  </div>
</div>""", unsafe_allow_html=True)
...
# ── Users ────────────────────────────────────────────────────────────────────
[users]
admin     = "pdm2026admin"      # prefix admin_ → Admin role
engineer  = "noc2026"           # prefix eng_   → Engineer role
viewer    = "readonly"          # prefix viewer_ → Viewer role
# admin_danaya = "secure-pw"    # additional admin
# eng_alice    = "alice-2026"   # additional engineer

# ── AI Chatbot Keys (add any you have — system tries in priority order) ───────
GROQ_API_KEY          = "gsk_..."     # console.groq.com  — free, fastest
ANTHROPIC_API_KEY     = "sk-ant-..."  # console.anthropic.com — highest quality
OPENROUTER_API_KEY    = "sk-or-..."   # openrouter.ai     — DeepSeek v3 free
HF_TOKEN              = "hf_..."      # huggingface.co    — Zephyr-7B free""", language="toml")
...
<div class="ac m">
  ...
  <span style="font-size:.74rem;color:#c9d1d9;line-height:1.7">
  <b style="color:#e6edf3">base_rul:</b> XGBoost v2 Final point prediction<br>
  <b style="color:#e6edf3">elapsed_min:</b> minutes since session start<br>
  <b style="color:#e6edf3">degrade_rate:</b> cycles/min per subsystem type<br>
  </span><br>
  <strong style="color:#f0b429">Urgency thresholds:</strong><br>
  <span style="font-size:.73rem;color:#c9d1d9">
  ...
  RUL > 50 → 🟢 Monitor (SLA ≤ 168h, Tier 1)
        ...
        with _lc2:
            st.markdown("""
<div class="ac m">
  <strong style="color:#39c5cf">Sensor Simulation</strong><br><br>
  <span style="font-size:.73rem;color:#c9d1d9;line-height:1.7">
  <code style="font-size:.70rem;color:#f0b429">val = nominal + d × elapsed × 0.0012 + N(0, nominal×0.013)</code><br><br>
  <b style="color:#e6edf3">nominal:</b> sensor operating setpoint<br>
  <b style="color:#e6edf3">d:</b> −1 (fail-low) or +1 (fail-high)<br>
  <b style="color:#e6edf3">N(μ,σ):</b> Gaussian noise, seeded by time bucket<br><br>
  Sparkline: 12 readings at 6-second intervals<br>
  Sensors: DC Voltage · Cabinet Temp · Fan Speed · VSWR ·<br>
  Latency · RSSI · CPU% · Battery Cap · ESR · PA Eff · Fade Margin
  </span>
</div>""", unsafe_allow_html=True)

        sh("PER-STATION DEGRADE RATES")
        _dr = '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:.35rem">'
        for s in STATIONS:
            _rn = live_rul(s); _dc = rc(_rn)
            _dr += (f'<div style="background:#161b22;border:1px solid #30363d;border-radius:5px;'
                    f'padding:.45rem .65rem;font-family:monospace;font-size:.70rem">'
                    f'<div style="color:#a5d6ff;font-weight:700">{s["id"]}</div>'
                    f'<div style="color:#7d8590;font-size:.61rem">{s["sub"].replace("_"," ")}</div>'
                    f'<div style="color:{_dc};font-weight:700">{_rn:.1f} cyc</div>'
                    f'<div style="color:#f0b429">{s["degrade"]:.2f} cyc/min</div>'
                    f'<div style="color:#7d8590;font-size:.62rem">{s["sensor_lbl"]} {s["sensor_unit"]}</div>'
                    f'</div>')
        _dr += '</div>'
        st.markdown(_dr, unsafe_allow_html=True)

    # ──────────────────────────────────────────────────────────────────────────
    with _ug_main[3]:
        _m_tabs = st.tabs(["Predictive", "RAG", "Agent & Ops", "C-MAPSS"])

        with _m_tabs[0]:
            sh("PREDICTIVE MODEL METRICS — XGBoost v2 Final")
            _pm = [
                ("RMSE","#ff6b35","14.60 all-4 · 12.77 FD001+FD003",
                 "Root Mean Squared Error in RUL cycles. Primary benchmark. Penalises large errors quadratically. "
                 "All-4=14.60 means average prediction error ≈14.6 cycles. "
                 "FD001+FD003=12.77 (single-condition, easier) vs FD002+FD004=16.43 (multi-condition, harder). "
                 "SOTA reference: CAELSTM=11.24 (FD001 only); gap = +1.53 cycles.",
                 "sqrt(mean((y_true - y_pred)²))","Lower is better. SOTA = 11.24 (FD001 single-subset only)."),
                ("MAE","#f0b429","9.97 all-4",
                 "Mean Absolute Error. Average absolute prediction error in cycles. "
                 "Less sensitive to outliers than RMSE. MAE=9.97 < RMSE=14.60 indicates some large outlier errors.",
                 "mean(|y_true - y_pred|)","Lower is better. MAE < RMSE = outlier errors present."),
                ("R²","#3fb950","0.874",
                 "Coefficient of Determination. Proportion of RUL variance explained. "
                 "R²=0.874 = 87.4% of variability captured. Improved from 0.853 (v1) via exp(α=3) weighting. "
                 "Scale-free (0–1), useful for cross-dataset comparison.",
                 "1 − SS_res / SS_tot","Higher is better. Perfect=1.0, null model=0.0."),
                ("NASA Score","#bc8cff","Supplementary asymmetric metric",
                 "Asymmetric scoring penalising late predictions (predict more RUL than actual = missed maintenance) "
                 "more heavily than early ones (unnecessary maintenance cost). "
                 "Reflects real-world maintenance risk asymmetry.",
                 "Σ exp(-d/13)-1 if d<0 else exp(d/10)-1 where d = y_pred - y_true","Lower is better."),
                ("Confidence Interval","#58a6ff","±3.13 cycles (1σ)",
                 "Prediction uncertainty band [cl, ch] per station via bootstrap. "
                 "Example: FD002_47 = [11.7, 17.7] for point prediction 14.7. "
                 "Wider CI = more uncertainty. Critical for scheduling maintenance within operational windows.",
                 "Bootstrap [P5, P95] of prediction distribution","Calibration: 68% of true RUL within ±1σ."),
                ("Per-RUL-Range RMSE","#39c5cf","0-20: 8.29 · 20-50: 18.64 · 50-100: 21.35",
                 "RMSE per urgency zone. XGBoost v2 performs best in the critical zone (0–20, RMSE=8.29) "
                 "due to exp(α=3) near-failure weighting. Most operationally important zone — "
                 "missing a critical-zone failure has the highest consequence.",
                 "RMSE computed per RUL band","Critical zone accuracy prioritised by design."),
            ]
            for mname, mcol, mval, mdesc, mform, minterp in _pm:
                with st.expander(f"{mname} = {mval}"):
                    st.markdown(f"""
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;font-size:.77rem">
  <div><div style="color:#7d8590;font-size:.62rem;text-transform:uppercase;font-family:monospace;margin-bottom:.25rem">DESCRIPTION</div>
  <div style="color:#c9d1d9;line-height:1.7">{mdesc}</div></div>
  <div><div style="color:#7d8590;font-size:.62rem;text-transform:uppercase;font-family:monospace;margin-bottom:.25rem">FORMULA & INTERPRETATION</div>
  <div style="color:#c9d1d9;margin-bottom:.4rem">{minterp}</div>
  <div style="background:#1c2333;border-radius:4px;padding:.35rem .55rem;font-family:monospace;font-size:.68rem;color:{mcol}">{mform}</div>
  </div>
</div>""", unsafe_allow_html=True)
        ...
        with _m_tabs[1]:
            ...
            _rm = [
                ("Grounding Rate","#3fb950","1.00 (100%)",
                 "Fraction of LLM-generated claims traceable to a retrieved [DOC-ID] evidence chunk. "
                 "Achieved via citation-enforced prompting: every factual assertion must cite a source. "
                 "Computed by expert annotation of 50 test cases.",
                 "Target >0.80 for production deployment."),
                ("Hallucination Rate","#ff6b35","0.00 (0%)",
                 "Fraction of LLM claims NOT supported by any retrieved evidence. "
                 "Reduced from 0.65 (Config C: LLM without RAG) to 0.00 (Config D+E) by RAG grounding. "
                 "This is the most critical quality metric for safety-critical maintenance.",
                 "Target <0.05. Reduction from 0.65→0.00 = RAG's primary contribution."),
                ("RAG Coverage Score","#39c5cf","0.60–1.00 per station",
                 "Fraction of top-5 retrieved chunks matching the alerted subsystem. "
                 "Coverage=1.00: all 5 chunks relevant. Coverage=0.60: backhaul stations (sparser corpus). "
                 "Low coverage triggers human review flag in diagnostic output.",
                 "power=1.00 · thermal=1.00 · RF=1.00 · BBU=1.00 · backhaul=0.60"),
                ("RRF Score (Reciprocal Rank Fusion)","#58a6ff","0.052–0.063 per chunk",
                 "RRF(k, r1, r2) = 1/(k+r1) + 1/(k+r2) with k=60, combining sparse (TF-IDF rank) "
                 "and dense (LSA rank) retrieval. Higher = stronger combined evidence. "
                 "17 candidates → top 5 selected by RRF.",
                 "Range: 0.04–0.07. Top chunk: ~0.063 for power-subsystem queries."),
                ("Retrieval Latency","#bc8cff","9ms (27.5ms of 33ms E2E)",
                 "Time from alert to evidence bundle (TF-IDF lookup + SVD transform + RRF fusion). "
                 "Dominates E2E pipeline at 83% of total latency. "
                 "Optimisation path: FAISS dense index would reduce to <1ms.",
                 "E2E breakdown: Interpreter 0.5ms · RAG 27.5ms · Diagnostic 0.8ms · Planning 0.2ms · Execution 2.4ms"),
            ]
            for mname, mcol, mval, mdesc, minterp in _rm:
                st.markdown(f"""
<div style="background:#161b22;border:1px solid #30363d;border-radius:6px;padding:.75rem .95rem;margin-bottom:.4rem">
  <div style="display:flex;align-items:baseline;gap:.55rem;margin-bottom:.28rem">
    <span style="font-weight:700;color:{mcol};font-family:monospace;font-size:.77rem">{mname}</span>
    <span style="background:{mcol}22;color:{mcol};border-radius:3px;padding:1px 6px;font-family:monospace;font-size:.65rem">{mval}</span>
  </div>
  <div style="font-size:.76rem;color:#c9d1d9;line-height:1.67;margin-bottom:.28rem">{mdesc}</div>
  <div style="font-size:.69rem;color:#7d8590;font-family:monospace;border-top:1px solid #30363d;padding-top:.25rem">{minterp}</div>
</div>""", unsafe_allow_html=True)

        with _m_tabs[2]:
            sh("AGENT & OPERATIONAL METRICS")
            _am = [
                ("Diagnostic Confidence","#58a6ff","0.37–0.92 per station",
                 "Probability assigned to primary root-cause hypothesis. "
                 "Derived from: RAG coverage × feature importance alignment × subsystem rule match. "
                 "High: FD002_47=0.88, FD003_88=0.91. Low: FD004_112=0.37 (sparse backhaul corpus).",
                 "Action threshold: >0.60 → auto-execute Tier 1/2. <0.50 → escalate."),
                ("Action Appropriateness","#bc8cff","88% on evaluation set",
                 "Fraction of recommended actions rated acceptable by maintenance engineers in expert review (50 cases). "
                 "12% disagreements mainly on AUTO/TIMEOUT tier boundary decisions.",
                 "Evaluated by domain experts against SOPs and alarm dictionaries."),
                ("Mean Time-to-Action","#39c5cf","33ms automated / <4h with human loop",
                 "Full pipeline from anomaly detection to action initiation: 33ms for Tier 1/2. "
                 "Tier 3 (Critical) adds human approval time (engineer on-call SLA).",
                 "Target: <100ms automated. Current: 33ms = well within threshold."),
                ("SLA Compliance","#3fb950","Critical ≤4h · Warning ≤48h · Monitor ≤168h",
                 "Time-based service level agreements per urgency tier. "
                 "Critical: field dispatch within 4 hours. Warning: inspection within 48 hours. "
                 "Monitor: add to scheduled maintenance queue within 168 hours (7 days).",
                 "SLAs enforced by TIMEOUT governance tier — auto-escalates if unapproved within window."),
                ("Governance Tier Distribution","#f0b429","Tier 1: 58% · Tier 2: 31% · Tier 3: 11%",
                 "Fraction of actions in each governance tier across the 15 monitored stations. "
                 "Most actions (58%) are fully autonomous (Monitor urgency). "
                 "11% require human approval (Critical urgency). "
                 "Designed to maximise autonomy while ensuring human oversight for high-stakes decisions.",
                 "Addresses governance gap identified in Allam et al. (2025)."),
            ]
            for mname, mcol, mval, mdesc, minterp in _am:
                st.markdown(f"""
<div style="background:#161b22;border:1px solid #30363d;border-radius:6px;padding:.75rem .95rem;margin-bottom:.4rem">
  <div style="display:flex;align-items:baseline;gap:.55rem;margin-bottom:.28rem;flex-wrap:wrap">
    <span style="font-weight:700;color:{mcol};font-family:monospace;font-size:.77rem">{mname}</span>
    <span style="background:{mcol}22;color:{mcol};border-radius:3px;padding:1px 6px;font-family:monospace;font-size:.65rem">{mval}</span>
  </div>
  <div style="font-size:.76rem;color:#c9d1d9;line-height:1.67;margin-bottom:.28rem">{mdesc}</div>
  <div style="font-size:.69rem;color:#7d8590;font-family:monospace;border-top:1px solid #30363d;padding-top:.25rem">{minterp}</div>
            ...
            sh("TOOL RISK TIERS")
            _tools = [
                ("query_cmdb","LOW","AUTO","Query equipment config, firmware, last maintenance"),
                ("search_knowledge","LOW","AUTO","Retrieve RAG evidence with citations"),
                ("open_ticket","LOW","AUTO","Create monitoring or warning ticket"),
                ("escalate_to_human","LOW","AUTO","Send alert notification to engineer"),
                ("schedule_dispatch","MED","TIMEOUT","Schedule field dispatch within SLA window"),
                ("remote_command","MED","TIMEOUT","OMC commands: TX power, fan speed"),
                ("remote_reboot","HIGH","HUMAN","Restart component via OMC (service risk)"),
                ("emergency_dispatch","HIGH","HUMAN","Immediate dispatch <4h (cost commitment)"),
                ("take_offline","HIGH","HUMAN","Graceful shutdown via OMC (service outage)"),
            ]
            _TH = "background:#1c2333;color:#7d8590;padding:.3rem .55rem;border:1px solid #30363d;font-size:.62rem"
            _TD = "padding:.27rem .55rem;border:1px solid #30363d;font-size:.70rem;font-family:monospace"
            _rows = "".join(
                f'<tr><td style="{_TD};color:#39c5cf">{t[0]}</td>'
                f'<td style="{_TD};color:{"#3fb950" if t[1]=="LOW" else "#f0b429" if t[1]=="MED" else "#ff6b35"}">{t[1]}</td>'
                f'<td style="{_TD};color:{"#3fb950" if t[2]=="AUTO" else "#f0b429" if t[2]=="TIMEOUT" else "#ff6b35"}">{t[2]}</td>'
                f'<td style="{_TD};color:#c9d1d9">{t[3]}</td></tr>'
                for t in _tools
            )
            st.markdown(f'<table style="border-collapse:collapse;width:100%;font-family:monospace"><tr>'
                        f'<th style="{_TH}">Tool</th><th style="{_TH}">Risk</th>'
                        f'<th style="{_TH}">Tier</th><th style="{_TH}">Description</th>'
                        f'</tr>{_rows}</table>', unsafe_allow_html=True)

        with _m_tabs[3]:
            sh("NASA C-MAPSS DATASET & DOMAIN MAPPING")
            st.markdown("""
<div style="background:#0d1117;border:1px solid #30363d;border-radius:8px;padding:.95rem 1.15rem;
     font-size:.78rem;color:#c9d1d9;line-height:1.75;margin-bottom:.8rem">
  NASA C-MAPSS (Saxena et al., 2008) is the community-standard RUL benchmark.
  TeleMaint AI uses it as a structured proxy for telecom BTS degradation, with explicit
  KPI mapping validated against established practice in the PdM literature.
</div>""", unsafe_allow_html=True)
            _TH2 = "background:#1c2333;color:#7d8590;padding:.32rem .6rem;border:1px solid #30363d;font-size:.62rem"
            _TD2 = "padding:.3rem .6rem;border:1px solid #30363d;font-size:.71rem;font-family:monospace"
            st.markdown(f"""
<table style="border-collapse:collapse;width:100%;margin-bottom:1rem">
  <tr>
    <th style="{_TH2}">Sub-dataset</th><th style="{_TH2}">Train</th><th style="{_TH2}">Test</th>
    <th style="{_TH2}">Conditions</th><th style="{_TH2}">Faults</th>
    <th style="{_TH2}">Difficulty</th><th style="{_TH2}">RMSE v2</th><th style="{_TH2}">Telecom Analogue</th>
  </tr>
  <tr><td style="{_TD2};color:#3fb950;font-weight:700">FD001</td><td style="{_TD2}">100</td><td style="{_TD2}">100</td>
      <td style="{_TD2}">1</td><td style="{_TD2}">1</td><td style="{_TD2};color:#3fb950">Easy</td>
      <td style="{_TD2};color:#3fb950;font-weight:700">12.31</td><td style="{_TD2};color:#7d8590">Single-environment urban BTS</td></tr>
  <tr><td style="{_TD2};color:#f0b429;font-weight:700">FD002</td><td style="{_TD2}">260</td><td style="{_TD2}">259</td>
      <td style="{_TD2}">6</td><td style="{_TD2}">1</td><td style="{_TD2};color:#f0b429">Medium</td>
      <td style="{_TD2};color:#f0b429;font-weight:700">15.87</td><td style="{_TD2};color:#7d8590">Multi-climate distributed fleet</td></tr>
  <tr><td style="{_TD2};color:#58a6ff;font-weight:700">FD003</td><td style="{_TD2}">100</td><td style="{_TD2}">100</td>
      <td style="{_TD2}">1</td><td style="{_TD2}">2</td><td style="{_TD2};color:#58a6ff">Medium</td>
      <td style="{_TD2};color:#58a6ff;font-weight:700">13.23</td><td style="{_TD2};color:#7d8590">Urban BTS — power + thermal faults</td></tr>
  <tr><td style="{_TD2};color:#ff6b35;font-weight:700">FD004</td><td style="{_TD2}">249</td><td style="{_TD2}">248</td>
      <td style="{_TD2}">6</td><td style="{_TD2}">2</td><td style="{_TD2};color:#ff6b35">Hard</td>
      <td style="{_TD2};color:#ff6b35;font-weight:700">16.99</td><td style="{_TD2};color:#7d8590">Multi-climate — mixed fault modes</td></tr>
</table>""", unsafe_allow_html=True)

            sh("DOMAIN MAPPING — C-MAPSS → TELECOM BTS")
            _maps = [
                ("21 sensor channels","Multivariate KPI streams (RSSI, SINR, throughput, power draw, temperature, battery voltage)",
                 "Both: correlated multivariate time-series capturing equipment health state"),
                ("Multiple operating conditions (FD002/FD004)","Geographic & environmental diversity (urban/rural, tropical/temperate, grid/solar)",
                 "Models must generalise across operating regimes varying in load, climate, power profile"),
                ("Run-to-failure trajectories","BTS component degradation (power unit, RF amplifier, backhaul, BBU)",
                 "Both exhibit gradual decline across multiple correlated channels before failure"),
                ("Piecewise linear RUL labelling","Maintenance window estimation for field dispatch scheduling",
                 "Both require estimating time-to-failure within operational windows"),
                ("Multiple fault modes (FD003/FD004)","Mixed BTS failures (power + thermal, RF + backhaul)",
                 "Models handle heterogeneous degradation signatures from distinct mechanisms"),
            ]
            for c_map, t_map, rationale in _maps:
                st.markdown(f"""
<div style="display:grid;grid-template-columns:1fr 1fr 1.1fr;gap:.6rem;padding:.4rem .65rem;
     background:#161b22;border:1px solid #30363d;border-radius:5px;margin-bottom:.22rem;
     font-size:.70rem;font-family:monospace">
  <div><span style="color:#7d8590">C-MAPSS: </span><span style="color:#58a6ff">{c_map}</span></div>
  <div><span style="color:#7d8590">Telecom: </span><span style="color:#39c5cf">{t_map}</span></div>
  <div><span style="color:#7d8590">{rationale}</span></div>
</div>""", unsafe_allow_html=True)

    # ──────────────────────────────────────────────────────────────────────────
    with _ug_main[4]:
        sh("DEPLOYMENT — STREAMLIT CLOUD (RECOMMENDED)")
        st.markdown("""
<div class="ac m">
  <strong style="color:#3fb950">Streamlit Cloud — Free, 25-second build, zero version conflicts</strong><br>
  ...
  2. Go to <a href="https://share.streamlit.io" style="color:#39c5cf">share.streamlit.io</a> → New app → select repo → set main file<br>
  3. App Settings → Secrets → paste keys (see AI Chatbot Setup tab)<br>
  4. Click Deploy — live in approximately 25 seconds
  </span>
</div>""", unsafe_allow_html=True)
        sh("REQUIREMENTS.TXT")
...
numpy==1.26.4
plotly==5.24.1
anthropic==0.40.0""", language="text")
        sh("LOCAL DEVELOPMENT")
        st.code("""# 1. Install
pip install streamlit numpy plotly anthropic

# 2. Create secrets
mkdir -p .streamlit && cat > .streamlit/secrets.toml << 'EOF'
GROQ_API_KEY       = "gsk_..."     # free at console.groq.com
ANTHROPIC_API_KEY  = "sk-ant-..."  # free at console.anthropic.com
[users]
admin    = "pdm2026admin"
engineer = "noc2026"
viewer   = "readonly"
...
# 3. Run
streamlit run streamlit_pdm.py""", language="bash")

        sh("WHY STREAMLIT INSTEAD OF HUGGINGFACE SPACES")
        _reasons = [
            ("Build time","HF Spaces: 30min–4h (gradio version conflicts)","Streamlit Cloud: ~25 seconds (no conflicts)"),
            ("Version stability","Gradio 4→5→6 all break different APIs","Streamlit 1.x backward-compatible API"),
            ("Chat API","type=messages removed in Gradio 6","st.chat_input/message: permanent API since v1.25"),
            ("Free tier","Yes, with cold starts","Yes, with cold starts"),
            ("Secrets","HF Space Secrets (env vars)","App Settings → Secrets (TOML format)"),
            ("Custom domain","huggingface.co/spaces/username/name","share.streamlit.io/username/repo"),
        ]
        _TH3 = "background:#1c2333;color:#7d8590;padding:.3rem .6rem;border:1px solid #30363d;font-size:.62rem"
        _TD3 = "padding:.28rem .6rem;border:1px solid #30363d;font-size:.71rem;font-family:monospace"
        _rows3 = "".join(
            f'<tr><td style="{_TD3};color:#c9d1d9;font-weight:700">{r[0]}</td>'
            f'<td style="{_TD3};color:#ff6b35">{r[1]}</td>'
            f'<td style="{_TD3};color:#3fb950">{r[2]}</td></tr>'
            for r in _reasons
        )
        st.markdown(f'<table style="border-collapse:collapse;width:100%"><tr>'
                    f'<th style="{_TH3}">Aspect</th><th style="{_TH3}">HuggingFace Spaces</th>'
                    f'<th style="{_TH3}">Streamlit Cloud</th></tr>{_rows3}</table>', unsafe_allow_html=True)
...
if st.session_state.live_mode:
    ...
    _ph.empty()
    st.rerun()
def _get_free_key(name):
    """Read free API key from secrets, env, or session state."""
    for source in [
        lambda: st.secrets[name],
        lambda: os.environ.get(name, ""),
        lambda: st.session_state.get(f"_rt_{name.lower()}", ""),
    ]:
        try:
            v = source()
            if v and len(str(v).strip()) > 10: return str(v).strip()
        except Exception:
            pass
    return ""

def _call_groq(key, messages, sys_p):
    try:
        import urllib.request, json as _j
        payload = _j.dumps({"model":"llama-3.3-70b-versatile",
    ...
    except Exception as e: return None, f"Groq: {str(e)[:150]}"

def _call_openrouter(key, messages, sys_p, model="deepseek/deepseek-chat-v3-0324:free"):
    try:
        import urllib.request, json as _j
        ...
        req = urllib.request.Request("https://openrouter.ai/api/v1/chat/completions",
            ...
            headers={"Authorization":f"Bearer {key}","Content-Type":"application/json",
                     "HTTP-Referer":"https://telemaint-ai.streamlit.app","X-Title":"TeleMaint AI"})
        with urllib.request.urlopen(req, timeout=30) as r:
            return _j.loads(r.read())["choices"][0]["message"]["content"].strip(), None
    except Exception as e: return None, f"OpenRouter: {str(e)[:150]}"

def _call_hf(token, messages, sys_p):
    try:
        import urllib.request, json as _j
        prompt = f"<|system|>\n{sys_p}\n"
        for m in messages[-4:]:
            ...
            prompt += f"{tag}\n{m['content']}\n"
        prompt += "<|assistant|>\n"
        payload = _j.dumps({"inputs":prompt,
            "parameters":{"max_new_tokens":700,"temperature":0.3,"return_full_text":False}}).encode()
    ...
    except Exception as e: return None, f"HF: {str(e)[:150]}"
