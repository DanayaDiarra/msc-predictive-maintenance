...
from agents import (AgentPipeline, DiagnosticAgent, PlanningAgent, ExecutionAgent,
                    rule_based_answer, SUBSYSTEM_KB)
...
STATIONS = [
    ...
    dict(id="FD002_14",  urgency="Critical",sub="power_subsystem",      sla=4,  cl=8.2,ch=14.1,  conf=0.920,gr=1.0,hal=0.0,cost=900,auto_n=2,to_n=1,hum_n=1,cov=1.0,doc="SOP-PWR-001",subset="FD002",cycles=312,hyp="Critical rectifier fault — DC bus voltage below 42V threshold",fc="48V DC rectifier module B",mech="Module B failure — Module A at 140% rated load",alm="PWR-001 + PWR-003 (rectifier failure)",a1="Isolate rectifier B and activate bypass via OMC",a1t="AUTO",a1tool="remote_command",a2="Emergency dispatch — dual rectifier replacement",a2t="HUMAN",a2tool="schedule_dispatch",base_rul=11.2,top_feat="voltage_rolling_mean",top_imp=0.0798,degrade=0.65,sensor_lbl="DC Voltage",sensor_nom=42.8,sensor_unit="V",sensor_dir="low"),
    ...
    for _pg in all_pages:
        ...
        if _is_act:
            st.markdown(f"""<style>div[data-testid="stSidebar"] button[data-testid="{_pk}"]{{
              background:#1a2744!important;border:1px solid #39c5cf!important;
              color:#e6edf3!important;font-weight:700!important;
              box-shadow:inset 3px 0 0 #39c5cf!important;}}</style>""", unsafe_allow_html=True)
        if st.button(f"{_icons.get(_pg,'')} {_pg}",key=_pk,use_container_width=True):
